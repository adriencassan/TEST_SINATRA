...
WECHAT_TOKEN = "6NmzSxrjuF4HLJZb6qpg1_2hwMtQM4EnqFNoEWWhTFQ"
...
class WeChatApiResource
  ...
  def post(request, params)
    ...
    if validate_message(message)
      anwser = WeChatApiResource.stupid_answer(message.xpath('//Content')[0].inner_html)
      content = "You typed: " << message.xpath('//Content')[0].inner_html
      return format_message(message,  anwser)
    end
  ...
  def self.stupid_answer(message)
  # REFACTOR.......
  if message == "Ok"
    return "Thanks ! Your box in on its way !"
  end
  item = ""
  qte = ""
  message.split(" ").each do |word|
    item = word if ['carrots','banana'].include?(word)
      #qte = /\d+/.match(word)[0].to_i
      qte = word[/\d+/].to_i if word.match(/\d+/)
    end
    return "Add to the database : #{qte} kg of #{item}"
    if !(item = "" || qte = "") then
      return "Add to the database : #{qte} kg of #{item}"
    else
